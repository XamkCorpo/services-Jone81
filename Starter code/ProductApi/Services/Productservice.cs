...
public class ProductService : IProductService
{
    ...
    public async Task<Result<List<ProductResponse>>> SearchByNameAsync(string searchTerm)
    {
        if (string.IsNullOrWhiteSpace(searchTerm))
            return Result.Failure<List<ProductResponse>>("Hakutermi ei voi olla tyhjä");

        try
        {
            List<Product> products = await _repository.SearchByNameAsync(searchTerm);
            List<ProductResponse> response = products.Select(p => p.ToResponse()).ToList();
            return Result.Success(response);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Virhe tuotteiden nimihakussa: {SearchTerm}", searchTerm);
            return Result.Failure<List<ProductResponse>>("Tuotteiden nimihaku epäonnistui");
        }
    }
}